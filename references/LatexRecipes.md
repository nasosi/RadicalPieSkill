...
| `a/b` on one line | `Fr (t='horz')` or `Fr (t='diag')` | Catalogue, `Fr` |
| `\binom{n}{k}` | `Fr (at)` inside a `Br` | Example 3 |
| `x^2`, `x_i`, both at once | `Sc` with `sups`, `subs` | Examples 8, 14 |
| `{}^{14}\mathrm{C}` | `Sc (pr)` before the symbol | Catalogue, `Sc` |
| `\sqrt{x}` | `Rd` | Example 8 |
...
| `\underset{k}{\circ}`, `\mathop{\circ}\limits_k` | `St` with a `'lowr'` label, which keeps the operator at its own size | Catalogue, `St` |
...
| `\begin{array}[t]`, `[b]`, `[c]` nested in one cell | `ba='frst'`, `'last'`, `'mddl'` on that matrix entry's group | Catalogue, `Mx` |
| `\overline`, `\underline` | `Wm (t='line')`, `un` for under; a `Bx` draws four sides or two and never one | Example 16 |
| `\vec` | `Wm (t='rarw')` | Example 16 |
| `\widehat`, `\widetilde` | `Wm (t='what')`, `Wm (t='tild')` | Catalogue, `Wm` |
...
| `\xleftrightarrow{}` | one `'long'` arrow, `u32{'left','larw','rght','rarw'}` | Catalogue, `Ar` |
...
| `\ce{<=>>}`, `\ce{<<=>}` | the same pair with one arrow short, `u32{'long','rght'}` or `u32{'left','long'}` | Catalogue, `Ar` |
| `\nrightarrow`, `\ce{-/>}` | one `'long'` arrow, `u32{'rarw','cent','slsh'}` | Catalogue, `Ar` |
...
| `\mspace{9mu}`, `\hspace{5pt}`, `\kern` | `Sp` with the width in math units, one math unit being the design font size over eighteen; nothing clamps the value | Catalogue, `Sp` |
...
| `\mathbb{R}`, `\mathcal{B}` and the rest | `st='doub'`, `'scpt'`, `'bdsc'` on the specific code point the alphabet gives that letter, `ℝ` not `R`; `st='frkt'`, `'sans'`, `'mono'` draw any letter | Example 18; catalogue, `Sb` — roles |
| `\mathbb{1}`, or any double-struck letter or digit outside ℕℤℚℝℂ | the Mathematical Alphanumeric code point, `𝟙` is U+1D7D9, with `st='doub'`; the code point alone with `st='uprt'` renders plain | Catalogue, `Sb` — roles |
| `\cdots` between factors | `Sb (ro='elps') {s{"⋯"}}`, with no operator dot before or after it | Catalogue, `Sb` — roles |
...
| `\boxed{x}` | `Bx` around the group | Catalogue, `Bx` |
| `\fbox{text}` | `Bx` around a `Sb (ro='text')`; the border thickness is `V (d='boxx',n='rule')` | Catalogue, `Bx` |
| A box open on two sides | `Bx (t='lwlf')`, `'lwrt'`, `'uplf'`, `'uprt'` | Catalogue, `Bx` |
| `\cancel{x}` | `Kt (t='lwup')` | Catalogue, `Kt` |
| `\bcancel{x}`, `\xcancel{x}` | `Kt (t='uplw')`, `Kt (t='exxx')` | Catalogue, `Kt` |
| `\sout{x}`, a rule straight through | `Kt (t='horz')` | Catalogue, `Kt` |
| `\cancelto{2}{4}`, or `\cancel{4}` with a replacement written above or below | the struck digit is `Kt (t='lwup')`; the replacement sits directly under or over it as a `St` whose main line is the struck digit and whose `Gr (t='lowr')` or `Gr (t='uppr')` holds the smaller replacement digit, usually sharing one colour with `pi=` | Catalogue, `St` — cancellation with a replacement |
| `\not` | `Ng` inside the `Sb` | Catalogue, `Ng` |
| `\phantom`, `\vphantom`, `\hphantom` | `Ph` with `t='full'`, `'vert'` which is the default, `'horz'` | Example 7 |
| `\mathmakebox[w][l]{...}`, a minimum width | `Ph (t='hzov')`, which reserves the box and lets what follows lie over it | Catalogue, `Ph` |
| A long division | `Dv` with a `quot` subgroup | Catalogue, `Dv` |
| A circled or boxed single symbol | `En` inside the `Sb` | Catalogue, `En` |
| `\phantom`, `\vphantom`, `\hphantom` | `Ph` with `t='full'`, `'vert'`, `'horz'` | Example 7 |
| A long division | `Dv` with a `quot` subgroup, the divisor written before it | Catalogue, `Dv` |
...
| A circled or boxed single symbol | `En` inside the `Sb`, fixed size and one per symbol | Catalogue, `En` |
| Label the terms of an equation (highlight, arrow, caption) | `Rt` or `Rr` behind it, `Ln` to a rail, `Gr (t='anno')` | Examples 23, 24; catalogue, `X` |
...
fitting the content, use the tab, `Sp (t)`, described in the catalogue under `Sp`.
